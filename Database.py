...
class Database:
    ...
    def add_ingredient(self) -> None:
        ...
        while getting_input != "N":
            ...
            if ingredient_name not in self.ingredients.keys():
                ...
                purchase_price = float(input("Enter cholesterol as percentage of daily value:\n-> "))
                purchase_quantity = float(input("Enter cholesterol as percentage of daily value:\n-> "))
                quantity_on_hand = float(input("Enter cholesterol as percentage of daily value:\n-> "))
    ...
    def sync_ingredients_from_google_sheets(self) -> None:
        ...
        for line in csvfile:
            if line[0] == "ingredient_name":
                ...
                quantity_on_hand = float(line[1])
                purchase_price = float(line[2])
                purchase_quantity = float(line[3])
                measurement = str(line[4])
                calories = float(line[5])
                fat = float(line[6])
                carbs = float(line[7])
                fiber = float(line[8])
    ...
    def sync_ingredients_from_csv(self) -> None:
        ...
        folder_directory = "C:/Users/18327/PycharmProjects/csci127-su22/Personal Projects/CSCI-127 Final/ingredients/"
        try:
    ...
    def sync_meals_to_csv(self) -> None:
        ...
        folder_directory = "C:/Users/18327/PycharmProjects/csci127-su22/Personal Projects/CSCI-127 Final/meals/"
        instruction_directory = os.path.join(folder_directory,
        ...
        with open(instruction_directory, "w+", newline="", encoding="utf-8") as user_meals:
            field_names = ["meal_name", "prep_time", "ingredient_1_name", "ingredient_1_quantity",
                           ...
                           "ingredient_5_name", "ingredient_5_quantity", "ingredient_6_name", "ingredient_6_quantity",
                           "ingredient_7_name", "ingredient_7_quantity", "ingredient_8_name", "ingredient_8_quantity"]
    ...
    def sync_meals_from_google_sheets(self) -> None:
        ...
        for line in csvfile:
            if line[0] == "meal_name":
                ...
                self.meals[meal_name] = Meal(meal_name=meal_name, prep_time=int(prep_time))
    ...
    def print_all_snacks_in_database(self) -> None:
        # TODO Might not needs this...
        count_meals = 0
        for meal in self.meals.keys():
            count_meals += 1
            if count_meals == 0:
                print("There are no snacks in the database.")
            else:
                this_meal = self.meals[meal]
                if this_meal.is_snack is True:
                    print(inflection.titleize(this_meal.meal_name))
    ...
    def add_meal(self) -> None:
        ...
        while getting_input != "N":
            ...
            if meal_name not in self.meals.keys():
                ...
                self.meals[meal_name] = Meal(meal_name, prep_time)
    ...
    def reload_meals(self) -> None:
        ...
        for meal in self.meals:
            ...
            del meal
            self.meals[meal_name] = Meal(meal_name, meal_prep_time)
            for new_ingredient in ingredients_to_add.keys():
                this_meal = self.meals[meal_name]
    ...
    def change_meal_to_snack(self) -> None:
        # TODO Might not use this snack feature...
        meal = input("Which meal would you like to make a snack?\n-> ").lower()
        if meal not in self.meals.keys():
            print("\'" + meal + "\' not found in database.")
        else:
            this_meal = self.meals[meal]
            this_meal.is_snack = True
    ...
    def generate_new_plans(self) -> None:
        ...
        while generating_new_plans is True:
            ...
            if number_of_meals > 0:
                ...
                self.plans = {}
                plan_number = 0
                for new_plan_index in range(10000):  # Generates a large number of plans
                    plan_number += 1
                    new_plan_index = Plan()
    ...
    def get_cheapest_plan(self) -> None:
        # TODO Maybe don't need this method...
        cheapest_plan = None
        for plan in self.plans:
            test_plan = self.plans[plan]
            cheapest_plan = test_plan
        for plan in self.plans:
            test_plan = self.plans[plan]
            if test_plan.cost < cheapest_plan.cost:
                self.cheapest_plan = test_plan
    ...
    def select_and_export_plan(self) -> None:
        ...
        # TODO Finish this... export to text file? Google Docs?
        selected_plan = None
        plan_id = int(input("Please input the number of the plan you want to export."))
        plan_found = False
        if self.cheapest_plan.index == plan_id:
            selected_plan = self.cheapest_plan
            print("Plan " + str(self.cheapest_plan.index) + " selected.")
            plan_found = True
        else:
            for plan in self.top_five_plans:
        ...
        if plan_found is True:
            print("\n======================== PLAN " + str(selected_plan.index) + " ========================")
            print("Total cost is: $" + str(round(selected_plan.cost, 2)))
            selected_plan.print_score()
            print("\nMeals contained:")
            for meal in selected_plan.meals:
                this_meal = selected_plan.meals[meal]
                print("\t" + str(selected_plan.quantities[this_meal]) + " x " + inflection.titleize(this_meal.meal_name))
    ...
    def get_ingredients_on_hand(self) -> None:
        # TODO Might not need this method
        self.ingredients_on_hand = {}
        for ingredient in self.ingredients:
            this_ingredient = self.ingredients[ingredient]
            self.ingredients_on_hand[this_ingredient] = this_ingredient.quantity_on_hand
        for ingredient in self.ingredients_on_hand:
            print(ingredient.ingredient_name + " : " + str(self.ingredients_on_hand[ingredient]))
...
""" add user and check login
add ingredient
add meal (shoot for 15)
add snack (shoot for 5)
create health/cost targets
create weekly menu based on targets
ability to shuffle weekly menu or choose from ranking
print weekly recipe instructions
print grocery list from weekly menu
ability to save/load database """
